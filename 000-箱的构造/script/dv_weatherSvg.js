...
//接收传递过来的参数
let iconDay = input.iconDay;
let windSpeedDay = input.windSpeedDay;
let windSpeedNight = input.windSpeedNight;

if(iconDay)
//天气动画
weathericon(iconDay,Math.max(windSpeedDay,windSpeedNight))
...
function weathericon(iconDay,windyspeed){
	...
	if(windyspeed>39)//6级大风
	{
		...
		if (cloudy_moon_arr.includes(weathernum))
			dv.paragraph(cloudy_moon)
		if (sunny_wind_arr.includes(weathernum))
			dv.paragraph(sunny_wind)
