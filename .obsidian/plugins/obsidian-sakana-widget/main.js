...
// sakanaWidgetIndex.ts
var sakanaWidgetIndex_exports = {};
__export(sakanaWidgetIndex_exports, {
  default: () => SakanaWidgetPlugin
});
module.exports = __toCommonJS(sakanaWidgetIndex_exports);
...
function getCanvasCtx(canvas, appSize, devicePixelRatio2 = (window.devicePixelRatio || 1) * 2) {
  const canvasRenderSize = appSize * devicePixelRatio2;
  ...
  ctx.scale(devicePixelRatio2, devicePixelRatio2);
  return ctx;
}

// component/wrapper/index.ts
var defaultOptions = {
  size: 200,
  autoFit: false,
  character: "chisato",
...
var _characters = {};
Object.keys(characters_default).forEach((key) => {
  const _char = characters_default[key];
  _characters[key] = cloneDeep(_char);
});
var _SakanaWidget = class {
  constructor(options = {}) {
    ...
    this._updateSize = (size2) => {
      ...
      this._draw();
      this._domMain.style.width = `${size2}px`;
      this._domMain.style.height = `${size2}px`;
      this._domImage.style.width = `${this._imageSize}px`;
      this._domImage.style.height = `${this._imageSize}px`;
      this._domImage.style.transformOrigin = `50% ${size2}px`;
    };
    this._updateDom = () => {
      ...
      app2.className = "sakana-widget-app";
      this._domApp = app2;
      wrapper.appendChild(app2);
      const canvas = document.createElement("canvas");
      canvas.className = "sakana-widget-canvas";
      this._domCanvas = canvas;
      app2.appendChild(canvas);
      ...
      github.innerHTML = github_default;
    ...
    this._draw = () => {
      ...
      const { size: size2, controls, stroke } = this._options;
      ...
      ctx.save();
      ctx.translate(this._canvasSize / 2, size2 + (this._canvasSize - size2) / 2);
    ...
    this._magicForce = () => {
      ...
      this._magicForceTimeout = window.setTimeout(this._magicForce, Math.random() * 3e3 + 2e3);
    };
    this.triggetAutoMode = () => {
      ...
      if (this._magicForceEnabled) {
        this._magicForceTimeout = window.setTimeout(this._magicForce, Math.random() * 1e3 + 500);
      }
    };
    this.setState = (state) => {
      if (!this._state) {
...
var SakanaWidget = _SakanaWidget;
...
var wrapper_default = SakanaWidget;
...
var SakanaWidgetPlugin = class extends import_obsidian.Plugin {
  async onload() {
  ...
  addSakanaWidget() {
    ...
    this.sakanaWidget = new wrapper_default({ character: this.settings.lastWidget ? this.settings.lastWidget : "chisato", autoFit: true }).setState({ i: 0.03, d: 0.99 }).mount("#sakana-widget");
  }
  async onunload() {
  ...
  async detachSakanaWidget() {
    if (this.sakanaBoxEl) {
      await this.saveCurrentWidgetName();
      this.sakanaBoxEl.detach();
      this.sakanaBoxEl = void 0;
      this.sakanaWidget = void 0;
    }
  }
  getCoverRealPath(imageUrl) {
    if (!imageUrl)
      return "";
  ...
  loadSakanaWidgets() {
    if (this.settings.widgets.length > 0) {
      this.settings.widgets.forEach((widget) => {
        const selfWidget = wrapper_default.getCharacter("chisato");
        if (selfWidget)
          selfWidget.image = this.getCoverRealPath(widget.url);
        if (selfWidget)
          wrapper_default.registerCharacter(widget.name, selfWidget);
      });
    }
  }
  async loadSettings() {
...
var SakanaWidgetSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Add new widget").setDesc("Create a new widget to play with ~ \u{1F41F}").addButton((button) => button.setButtonText("+").onClick(async () => {
  ...
  displayMacroSettings() {
    ...
    settings.widgets.forEach((widget, index) => {
      const topLevelSetting = new import_obsidian.Setting(containerEl).setClass("widget-setting").setName(`SakanaWidget #${index + 1}`).addButton((button) => button.setButtonText("Delete Widget").onClick(async () => {
      ...
      mainSettingsEl.createEl("input", {
        ...
        type: "text",
        value: widget.name
      }).on("change", ".name-input", async (evt) => {
